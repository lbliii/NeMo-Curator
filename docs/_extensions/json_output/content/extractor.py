...
def extract_document_content(env: BuildEnvironment, docname: str, content_cache: dict) -> dict[str, Any]:
    ...
    try:
        ...
        fast_extraction = (
            getattr(config, "json_output_settings", {}).get("fast_text_extraction", False) if config else False
        )
        lazy_extraction = (
            getattr(config, "json_output_settings", {}).get("lazy_extraction", False) if config else False
        )
        skip_complex = (
            getattr(config, "json_output_settings", {}).get("skip_complex_parsing", False) if config else False
        )
        ...
        try:
            ...
            filter_clutter = (
                getattr(config, "json_output_settings", {}).get("filter_search_clutter", True) if config else True
            )
