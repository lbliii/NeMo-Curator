...
| Stage | Behavior |
| --- | --- |
| `RegexSubstitutionStage` | Applies ordered regular-expression rules from a YAML list, then collapses whitespace. It writes an empty-result skip reason only when nonempty input becomes empty. |
| `AbbreviationConcatStage` | Joins matched ASR-spelled letter sequences, such as `N V I D I A` to `NVIDIA`, in the selected language. |
| `WhisperHallucinationStage` | Flags repeated words, long words, maintained corpus phrases, or a high character rate. It writes a `Hallucination:` value to `_skipme` and the matching checks to `additional_notes`. |
| `SelectBestPredictionStage` | Selects a configured primary, fallback, or reference field according to policy. By default, it writes the text to `best_prediction` and the route to `best_prediction_source`. The selection does not measure transcription accuracy or make model output ground truth. |
...
For this example, each manifest record includes `pred_text`, `duration`, `_skipme`, and `language`. `language` selects the language-aware long-word rule. The hallucination stage treats a missing language as empty. `AbbreviationConcatStage` defaults to `source_lang` and English. Set `source_lang_key="language"` to share the manifest language field. Refer to the maintained [hallucination pipeline configuration](https://github.com/NVIDIA-NeMo/Curator/blob/main/tutorials/audio/whisper_hallucination/pipeline.yaml) and [phrase corpus](https://github.com/NVIDIA-NeMo/Curator/blob/main/tutorials/audio/whisper_hallucination/phrases.txt) for the complete inputs.
...
While `AudioToDocumentStage` converts audio data to `DocumentBatch` format, the transcript stages in [Process Transcripts Before Conversion](#process-transcripts-before-conversion) handle common ASR cleanup and screening on `AudioTask` records. Implement a custom stage on the converted `DocumentBatch` data for transformations outside those stages.
