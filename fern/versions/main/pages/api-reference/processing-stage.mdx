...
def inputs(self) -> StageInputSpecs:
    ...
    Returns:
        A tuple of (required_task_attributes, required_data_attributes), or a
        mapping from supported task types to those tuples.
    """
```

Return one tuple when every supported task type has the same requirements. To
declare different requirements for different task types, return a mapping keyed
by task type. For example, a stage can require the `data` attribute from a
`FileGroupTask` and a `text` column from a `DocumentBatch`:

```python
from nemo_curator.stages.base import StageInputSpecs
from nemo_curator.tasks import DocumentBatch, FileGroupTask

def inputs(self) -> StageInputSpecs:
    return {
        FileGroupTask: (["data"], []),
        DocumentBatch: (["data"], ["text"]),
    }
```
...
The framework infers fanout when `process()` has a return annotation containing
`list[OutputT]`. This includes unions such as `OutputT | list[OutputT] | None`.
