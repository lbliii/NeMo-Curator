...
| `model_inference_batch_size` | `int \| None` | `8192` | Maximum number of texts sent to vLLM in each embedding call. Set to `0` or `None` to process the entire input task in one call. Negative values are rejected. |
