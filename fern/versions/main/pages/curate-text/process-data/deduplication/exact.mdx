...
| `normalize_text` | bool | False | Whether to lowercase text, collapse whitespace runs, and trim leading and trailing whitespace for matching. The workflow preserves the original text. |
...
To control the fields written by `TextDuplicatesRemovalWorkflow`, use `output_fields`. The default `drop_id_field=False` preserves the deduplication ID. Set it to `True` to omit the ID. The workflow rejects configurations that include the ID in `output_fields` while dropping it.
