...
| `cache_kwargs` | dict | `{}` | Storage options for intermediate files and duplicate-ID artifacts, including those under `output_path`. |
| `write_kwargs` | dict | `{}` | Storage options for final deduplicated output and reading duplicate-ID artifacts. |
