---
description: "Read and write interleaved image-text datasets between WebDataset tar shards and Parquet, and read Lance datasets"
categories: ["text-curation"]
tags: ["interleaved", "io", "parquet", "webdataset", "lance", "reader", "writer", "schema"]
...
Read and write interleaved image-text data between WebDataset tar shards and Parquet, and read row-wise interleaved Lance datasets. The file readers materialize data as `InterleavedBatch` objects:
...
Interleaved samples can live in three on-disk formats. In memory, all materialize as `InterleavedBatch`:
...
| **Lance rows** | One row per item, grouped by `sample_id` | Versioned reads of interleaved datasets |
...
`InterleavedLanceReader` reads rows using the interleaved schema into validated
`InterleavedBatch` objects. Input rows for each `sample_id` must be consecutive. The reader does
not reorder rows, and it does not regroup samples across fragment partitions. Install the optional
Lance dependency with `pip install "nemo-curator[lance]"`.

```python
from nemo_curator.stages.interleaved.io import InterleavedLanceReader

reader = InterleavedLanceReader(path="./interleaved.lance")
```

Keep the required interleaved columns when selecting `fields`. Refer to the [text Lance reader](/curate-text/load-data/read-existing)
for dataset and storage options.

### Reader Parameters

The Parquet and WebDataset readers use these parameters:
