...
`CommonCrawlWarcIterator` reads WARC files from local paths or fsspec-compatible remote paths such as `s3://`. Pass credentials and other filesystem settings through `storage_options`; install the filesystem backend required by the path, such as `s3fs` for S3.
