---
description: "Read existing JSONL, Parquet, and Lance datasets using Curator's reader stages."
categories: ["how-to-guides"]
tags: ["jsonl", "parquet", "lance", "data-loading", "reader", "pipelines"]
...
<Tab title="Lance Reader">
:sync: lance

Install the optional Lance dependencies before using this reader:

```bash
pip install "nemo-curator[lance]"
```

Add a Lance reader to your pipeline:

```python
from nemo_curator.stages.text.io.reader import LanceReader

reader = LanceReader(
    path="/path/to/dataset.lance",
    fields=["text", "url"],
)
pipeline.add_stage(reader)
```

Use `fields` to select columns. Pass Lance dataset options such as remote-storage credentials
through `read_kwargs`, for example `read_kwargs={"storage_options": {...}}`.
