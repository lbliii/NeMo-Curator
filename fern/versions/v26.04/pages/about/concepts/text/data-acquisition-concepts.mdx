...
Extracts individual records from downloaded containers. You should only override `iterate` and `output_columns` where `iterate` must have logic to load the local file path and return bunch of documents. Records are passed directly to the extractor (if provided) inline during iteration.
