...
_AMPERE_COMPUTE_CAPABILITY_MAJOR = 8
...
def set_nemotron_parse_attention_backend(engine_kwargs: dict[str, Any]) -> None:
    """Select Triton where Nemotron-Parse is incompatible with vLLM's default."""
    if "attention_backend" in engine_kwargs or not torch.cuda.is_available():
        return

    capability_major = torch.cuda.get_device_capability()[0]
    if capability_major == _AMPERE_COMPUTE_CAPABILITY_MAJOR:
        engine_kwargs["attention_backend"] = "TRITON_ATTN"
        return

    if capability_major != _BLACKWELL_COMPUTE_CAPABILITY_MAJOR:
        return
